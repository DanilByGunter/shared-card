...
public class ImplGroupRepository implements GroupRepository{
    ...
    @Override
    public void createGroupFromList(GroupNameEntity groupName, List<UserNameEntity> users, List<Integer> statuses) {
        ...
        for(int i=0;i<users.size();i++){
            ...
            groupEntities.add(new GroupEntity(users.get(i).getId(),groupName.getId(),status));
        }
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
