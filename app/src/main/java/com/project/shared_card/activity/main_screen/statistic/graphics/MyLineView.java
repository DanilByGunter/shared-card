...
public class MyLineView extends View {
    ...
    private final int BACKGROUND_LINE_COLOR = getColor(getContext(), R.color.cold_green_gray);
    private final int BOTTOM_TEXT_COLOR = getColor(getContext(), R.color.light_green_2);
        ...
        if (dataLists != null && !dataLists.isEmpty()) {
            if (drawDotLists.size() == 0) {
                for (int k = 0; k < dataLists.size(); k++) {
                    drawDotLists.add(new ArrayList<MyLineView.Dot>());
                }
            }
            for (int k = 0; k < dataLists.size(); k++) {
    ...
    private void drawPopup(Canvas canvas, float num, Point point, int PopupColor) {
        ...
        NinePatchDrawable popup =
                (NinePatchDrawable) getDrawable(getContext(), R.drawable.popup_white);
    ...
    private void drawDots(Canvas canvas) {
        ...
        smallCirPaint.setColor(getColor(getContext(), R.color.dark_green));
